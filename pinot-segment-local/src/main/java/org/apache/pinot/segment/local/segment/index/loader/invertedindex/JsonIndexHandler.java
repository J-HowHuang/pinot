...
public class JsonIndexHandler extends BaseIndexHandler {
  ...
  public IndexAction getIndexChange(String column, SegmentDirectory.Reader segmentReader) {
    boolean existing = segmentReader.hasIndexFor(column, StandardIndexes.json());
    boolean desired = _jsonIndexConfigs.containsKey(column);
    if (existing && !desired) {
      return IndexAction.REMOVE;
    }
    if (!existing && desired) {
      ColumnMetadata columnMetadata = _segmentDirectory.getSegmentMetadata().getColumnMetadataFor(column);
      return shouldCreateJsonIndex(columnMetadata) ? IndexAction.ADD : null;
    }
    return null;
  }
